...
class TeamsFromUsersPagingSourceImpl(
    ...
    private val filters: TeamFilterModel,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TeamFromUserDto> {
        return try {
            ...
            val response =
                teamsFromUsersRemoteDataSource.getTeamsFromUsersList(page = page, uid = filters.uid)

            LoadResult.Page(
                data = response,
                prevKey = if (page == 1) null else page.minus(1),
                nextKey = if (filters.uid.isNotEmpty() || response.isEmpty()) null else page.plus(1)
            )
        } catch (e: Exception) {
            LoadResult.Error(e)
        }
